...
function checkEmptyField(f){
    return (f !== undefined && f != null && f != '');
}

function getMenuName(page_type){
    if(page_type == 'history'){
        return {'menu_name_th':'เกี่ยวกับหน่วยงาน', 'sub_menu':{'menu_name_th':'ประวัติความเป็นมา', 'menu_name_en':'History'}};
    }
    if(page_type == 'authority'){
        return {'menu_name_th':'เกี่ยวกับหน่วยงาน', 'sub_menu':{'menu_name_th':'อำนาจหน้าที่', 'menu_name_en':'Authority'}};
    }
    if(page_type == 'palace-minister'){
        return {'menu_name_th':'เกี่ยวกับหน่วยงาน', 'sub_menu':{'menu_name_th':'ทำเนียบรัฐมนตรี', 'menu_name_en':'Palace Minister'}};
    }
    if(page_type == 'palace'){
        return {'menu_name_th':'เกี่ยวกับหน่วยงาน', 'sub_menu':{'menu_name_th':'บุคลากร', 'menu_name_en':'Palace Minister'}};
    }
    if(page_type == 'law'){
...
function concatDateTimeSQL(d , t){
    ...
    // console.log(d);
    // return d;
    return d.getFullYear() + '-' + padLeft(""+(d.getMonth() + 1), '00') + '-' + padLeft(""+(d.getDate()), '00') + ' ' + t.getHours() + ':' + t.getMinutes() + ':00';
}
...
function convertDateToFullThaiDateIgnoreTime(dateObj){
    var lang = sessionStorage.getItem('DEFAULT_LANGUAGE');
    var dateTxt = '';
    var monthTxt = '';
    ...
    if(lang == 'TH'){
        ...
        switch(dateObj.getDay()){
            case 5 : dateTxt = 'Friday';break;
            case 6 : dateTxt = 'Saturday';break;
            case 0 : dateTxt = 'Sunday';break;
            case 1 : dateTxt = 'Monday';break;
            case 2 : dateTxt = 'Tuesday';break;
            case 3 : dateTxt = 'Wendesday';break;
            case 4 : dateTxt = 'Thursday';break;
        } 
        switch(dateObj.getMonth()){
...
function getHourList()
{
    ...
    for(var i = 0 ; i < 24; i++){
        ...
        hourList.push({'value': h, 'display' : i});
    }
    return hourList;
}
...
function getMinuteList()
{
    ...
    for(var i = 0 ; i < 60; i++){
        ...
        minuteList.push({'value': m, 'display' : i});
    }
    return minuteList;
}
...
app.filter('FindProductRate', function () {
    return function (input, val) {
        if (input !== undefined && input !== null) {
            var i = 0, len = input.length;
            for (; i < len; i++) {
                //console.log(input[i].UserID, '==' ,val);
                if (input[i].product_desc == val) {
                    return i;
                }
            }
            return -1;
